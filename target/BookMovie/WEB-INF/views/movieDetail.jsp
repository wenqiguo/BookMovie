    ...
    <script>
        ...
        function initBanner(){
            ...
            $.ajax({
                ...
                success:function (obj) {
                    var totalComments =obj.commentList;
                    StonefontTemp = obj.data.movieTotalticket;

                    if (obj.data.movieState==0){
                        StonefontTemp= "暂无"
                    }else{
                        StonefontTemp += "万";
                    }
                    ...
                    scoreNum.append("<span class=\"stonefont\">" +  totalComments.length + "</span>人评分");
        ...
        function intitActor(){
            ...
            $.ajax({
                ...
                success:function (obj) {
                    console.log(obj);
                    PictureUrl = "<%=basePath%>static/static/movies/" + obj.data.movieCname + "/director/" + obj.data.movieDirector + ".jpg";
                    // 导演
                    ...
                    //例如 巩俐:郎平,黄渤:陈忠和,彭昱畅:青年陈忠和
                    StrActor = obj.data.movieActor.split(',');
                    for(var i = 0;i<StrActor.length;i++){
                        StrRole = StrActor[i].split(':');
                        // StrRole[0]演员名字，StrRole[1]角色名字
                        //演员图片存储路径
                        PictureUrl = "<%=basePath%>static/static/movies/" + obj.data.movieCname + "/actors/" + StrRole[0] + ".jpg";
                        // 演员
                        PosName="&nbsp";
                        if(i==0){
                            PosName = "演职人员";
