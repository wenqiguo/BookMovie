...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="<%=basePath%>/static/static/css/pay.css">
...
<body>
    ...
    <script>
        ...
        {
           localStorage.setItem("minute","14");
        ...
        function initPay(){
            ...
            $.ajax({
                ...
                success:function (obj) {
                    ...
                    orderTable.append(
                        "<tr>" +
                            "<td class=\"movie-name\">" + obj.schedule.scheduleMovie.movieCname + "</td>" +
                            "<td class=\"showtime\">" + obj.schedule.scheduleStartTime + "</td>" +
                            "<td class=\"cinema-name\">" + obj.schedule.scheduleCinema.cinemaName + "</td>" +
                            "<td>" +
                                "<span class=\"hall\">" + obj.schedule.scheduleHall.hallName + "</span>" +
                                "<div class=\"seats\">" +
            ...
            setInterval(function(){
                ...
                if(second==0 && minute==0){
                    ...
                    for (var i = 0;i<localStorage.length;i++){
                        localStorage.removeItem(localStorage.getItem("minute"));
                        localStorage.removeItem(localStorage.getItem("second"));
                        localStorage.removeItem(localStorage.getItem("order"));
                    }
                    //关闭当前页面
                    window.opener = null;
                    window.open('', '_self');
        ...
        payBtn[0].onclick = function(){
            ...
            $.ajax({
                type: "post",
                url: url + "/order/buyMovieTickets.do",
                data: {
                    scheduleId: schedule_id,
                    position: position,
                    orderPrice: price,
                    ...
                    if(obj.type == "success"){
                        layui.use(['layer'], function(){
                            ...
                            layer.alert('购票成功！',{icon: 0,offset: clientHeight/5},
                                 function (){
                                    layer.closeAll();
                                    document.location.href = "/order/payStatus.do";
                                 }
                            );
                        });
                    }else{
